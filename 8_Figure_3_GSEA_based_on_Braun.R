...
correlation <- column_wise_correlation(set1 = expression_braun[,-1], 
                                       set2 = "gene", 
                                       gene = "COL7A1",
                                       normalization = "none", 
                                       return = "data")
...
for(d in 1:length(dbs_names)){
  ...
  if(!dir.exists(paths = paste0("CSV/GSEA_MsigDB_BRAUN/",dbs_names[d]))){
    dir.create(path = paste0("CSV/GSEA_MsigDB_BRAUN/",dbs_names[d]), recursive = T)
  }
  
  if(!dir.exists(paths = paste0("RDS/GSEA_MsigDB_BRAUN/",dbs_names[d]))){
    dir.create(path = paste0("RDS/GSEA_MsigDB_BRAUN/",dbs_names[d]), recursive = T)
  }
  ...
  saveRDS(object = GSEA_results, file =  paste0("RDS/GSEA_MsigDB_BRAUN/",dbs_names[d],"/", str_sub(str_split_fixed(dbs[d], "/", Inf)[,3], 1, -5), ".RDS")) 
  ...
  if(dim(GSEA_results_tibble)[1]!=0){
    write_csv(x = GSEA_results_tibble, file = paste0("CSV/GSEA_MsigDB_BRAUN/",dbs_names[d],"/", str_sub(str_split_fixed(dbs[d], "/", Inf)[,3], 1, -5), ".csv")) 
  }
}
